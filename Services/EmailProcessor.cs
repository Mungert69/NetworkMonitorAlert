...
public class EmailProcessor : IEmailProcessor
{
    ...
    public async Task<ResultObj> SendHostReport(HostReportObj hostReport)
    {
        ...
        var contentMap = new Dictionary<string, string>
            {
                { "EmailTitle", "Weekly Quantum Network Monitor Host Report" },
                 { "HeaderImageUrl",  headerImageUrl}, // Assuming this is your logo URL
                { "HeaderImageAlt", "Quantum Network Monitor Logo" },
              { "MainHeading", "Quantum Network Monitor Host Report" },
    ...
    public async Task<List<ResultObj>> UserHostExpire(List<GenericEmailObj> emailObjs)
    {
        ...
        foreach (var emailObj in emailObjs)
        {
            ...
            else
            {
                var contentMap = new Dictionary<string, string>
                    {
                 { "EmailTitle", "Action Required: Your Quantum Network Monitor Account"},
                { "HeaderImageUrl",  BuildUrl(emailObj as IGenericEmailObj)},
                { "HeaderImageAlt", emailObj.HeaderImageAlt },
                 { "MainHeading", "We Miss You at Quantum Network Monitor!" },
                  {"MainContent", $"Hello! Your free plan has temporarily paused monitoring on these hosts due to inactivity: {emailObj.ExtraMessage}. This helps us manage resources for active users.To keep your services running smoothly, please log in as soon as possible and reactivate monitoring. Consistent monitoring is important for security and customer satisfaction! <br></br><br></br>Instructions for re-enabling hosts:<br></br>Click Reactivate My Hosts below. This will take you to the Quantum Network Monitor Dashboard.<br></br>On the Quantum Network Monitor Dashboard Click the Login button, make sure to use this email address when logging in. You can login with Google, LinkedIn or create an email account<br></br>After you login you will see a list of your hosts {emailObj.ExtraMessage}. Tick enabled for all the hosts you want to enable<br></br>Click the save icon. Your hosts are now re-enabled. Your full alerting service is again active and you can rest assured your customers will always be served."},
                   { "ButtonUrl", $"{AppConstants.FrontendUrl}/dashboard" },
    ...
    public async Task<List<ResultObj>> UserProcessorExpire(List<GenericEmailObj> emailObjs)
    {
        ...
        foreach (var emailObj in emailObjs)
        {
            ...
            else
            {
                var contentMap = new Dictionary<string, string>
                    {
                        { "EmailTitle", "Action Required: Your Quantum Network Monitor Agent"},
                        { "HeaderImageUrl",  BuildUrl(emailObj as IGenericEmailObj)},
                        { "HeaderImageAlt", emailObj.HeaderImageAlt },
                        { "MainHeading", "Quantum Network Monitor Agent inactive!" },
                        { "MainContent", $"Hello! Your Quantum Network Monitor Agent has been temporarily paused on these hosts due to inactivity: {emailObj.ExtraMessage}. This action helps us allocate resources effectively to active agents. To ensure uninterrupted services and maintain security, please reactivate your agent by re-authenticating at your earliest convenience.<br></br><br></br>Instructions for reactivating your agent:<br></br>1. Open the Quantum Network Monitor Agent app on the device where it was originally installed, if you no longer have the agent instaled and want to continue using the service then download and install the app on the same, or a new, device using the same email address.<br></br>2. Follow the prompts to authorize your agent, similar to the initial setup process.<br></br>Once reactivated, monitoring will resume for the previously assigned hosts, and your full alerting service will be restored, ensuring your customers continue to receive optimal service.<br></br>To avoid having your agent deactivated simply run the agent for at least six hours once every three months."},
                        { "ButtonUrl", $"{AppConstants.FrontendUrl}/dashboard" },
